...
2. The fine-tune work and the app work are mostly parallel — they don't compete for the same hours, since the app shell can be developed against the prebuilt `litert-community/gemma-4-E4B-it-litert-lm` while the fine-tune trains.
...
Gemma 4 E4B, quantized INT4, distributed as `litert-community/gemma-4-E4B-it-litert-lm`. Approximate file size: **3.66 GB**. We ship one variant only — no E2B fallback. The trade-off: on the cheapest phones (2 GB RAM, MediaTek A22-class CPUs) the app won't run, and on $80–100 phones it will run but slowly. We accept this because shipping two model variants doubles the conversion and evaluation work for marginal coverage gain.

**Performance expectations** (from published LiteRT-LM benchmarks for Gemma 4 E4B):
- Galaxy S26 Ultra: ~17–18 tok/s CPU decode, ~22 tok/s GPU
- Raspberry Pi 5 (CPU only): ~3 tok/s
- Target hardware ($80–100 Android phone, CPU only): planning estimate 3–10 tok/s — not officially benchmarked
...
2. **Merge LoRA** into base Gemma 4 E4B weights → full Hugging Face safetensors
...
The pipeline is validated on **base Gemma 4 E4B first** (Conversion Track Phase A below), independently of and in parallel with app development, so we know the toolchain works before fine-tuned weights become the critical path.
...
Gemma 4 E4B is natively multimodal, so the same model handles all three input modes. We're not training new encoders.

### Target hardware

- **minSdk 28** (Android 9.0, 2018)
- **targetSdk 35**
- **Documented minimum**: 4 GB RAM, ~4 GB free storage
- **Demo recording device**: 6 GB+ RAM strongly preferred so OOM isn't a demo failure mode. A 3.66 GB model plus app and system overhead is tight on 4 GB. Acquiring this device is a Phase 1 dependency.
...
- **Not shipping a smaller E2B variant.** One model, one binary, one download.
...
#### Phase A — Validate `litert-torch export_hf` on base Gemma 4 E4B

Run the full conversion pipeline on base Gemma 4 E4B weights from Hugging Face. The output should be byte-identical-in-spirit to the prebuilt `litert-community/gemma-4-E4B-it-litert-lm` (same architecture, same quantization).

**Definition of done:**
- `litert-torch export_hf` runs to completion on base Gemma 4 E4B safetensors
- Output `.litertlm` file produced, ~3.66 GB, loads in the LiteRT-LM Android library
...
Once the Apiary fine-tune is trained: merge LoRA adapters into base weights, run the same conversion script, produce `apiary-4-E4B-int4.litertlm`.
...
3. **The model runs too slowly on the target $80–100 phone to feel usable.** 3 tok/s in the worst case means a 200-token response takes ~70 seconds. Mitigation: pre-warm model on app launch, stream tokens as proof of life, frame the wait as "thinking" with a progress indicator. Record demo on the 6 GB device first, where decode is closer to 10+ tok/s.
...
5. **Demo phone OOM kills the app mid-recording.** 3.66 GB model on a 4 GB device leaves almost no headroom. Mitigation: insist on the 6 GB+ demo device. Add a debug log of peak memory during model load + first query so we know how close to the edge we're running.
...
- **Model format**: `.litertlm`, INT4 quantized, ~3.66 GB
...
BeeCare Anywhere is the Android shell that puts Gemma Apiary in a beekeeper's hand — a $80 phone, 3.66 GB on disk, no network connection required, text + photo + audio in, regional diagnostic advice out, with a one-file swap to upgrade from base Gemma to the fine-tuned model the day after our hackathon submission.
...
- [E4B LiteRT-LM model card](https://huggingface.co/litert-community/gemma-4-E4B-it-litert-lm)
